# bill (공공에너지 전기 관리 대시보드)

## 한 줄 설명
종로도서관 공공에너지 전기 사용량과 요금을 한 화면(원페이퍼·무스크롤)에서 보는 대시보드. 실측 데이터 + 실제 날씨(Open-Meteo)로 연말 예산 전망·작년 대비 증감·날씨 기반 해명·행동 지침을 제공. PDF/이미지 전기요금 고지서는 Cloudflare Worker OCR로 자동 입력.
...
- `D['전기'][year][month] = {u:사용량, a:요금원}` — 전기 kWh.
- `D['전기'].budget[year]` — **105,600,000원** (연간 전기 예산, 실제값).
...
- `detectAnomalies` 기대치 대비 ±18% 벗어나면 전기 사용 급증/급감 / `forecastNext` 다음달 예측.
- `composeExplain` 작년比 **요금** |7%|↑ 달을 **사용량 vs 단가** 조합 + 검침기간 실제 기온으로 한 문장 해명. 예: "추워서 많이 써서 요금 10%↑(단가는 비슷)".

## 화면 구성 (원페이퍼)
- **상단 좌**: AI 분석·조언 (일침 배너 + 권장조치 카드). 결정적 엔진으로 동기 렌더.
- **상단 우 (탭 패널)**: `📊 월별 누적`(예산 게이지, 작년比 ▲▼% 배지) / `📷 PDF 업로드`(드롭존→OCR) / `✏️ 입력표`(실시간 편집).
  - **입력표 셀 수정 → setVal → render() → 게이지·차트·AI·분석띠 즉시 연동.**
- **하단**: 월별 사용량 차트(작년 좌·올해 우, 급증=빨강, 예측=빗금, 월별 작년比 ▲▼% 배지) + **'이번 분석' 띠**(`composeExplain` 결과를 전체폭 문장으로).
  - **전기 차트엔 기온 점**(작년=회색·올해=주황, 우측 °C축 `y2`), 같은 달끼리 점선 연결(`tempLink` 플러그인). 기온표를 차트로 흡수해 비운 공간에 분석 띠를 넓게 배치.
